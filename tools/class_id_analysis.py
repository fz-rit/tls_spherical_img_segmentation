...
def plot_class_id_hist(class_id_vec, class_map, prefix = 'train', save_path="class_id_histogram.png"):
    ...
    # fig.tight_layout()
    plt.savefig(f"{prefix}_{save_path}", dpi=300)
    plt.show()

DATA_SPLIT = 'test'

csv_file_dir = Path(f"/home/fzhcis/mylab/data/point_cloud_segmentation/palau_2024_for_rc/{DATA_SPLIT}/csv")
csv_file_paths = list(csv_file_dir.glob("*.csv"))
class_id_ls, class_id_all = grab_class_id(csv_file_paths)
plot_class_id_hist(class_id_all, CLASS_MAP, prefix=DATA_SPLIT, save_path="class_id_histogram.png")
# for i in range(len(class_id_ls)):